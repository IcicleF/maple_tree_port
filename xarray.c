...
/* Move the radex tree node cache here */
extern struct kmem_cache *radex_tree_node_cachep;
extern void radex_tree_node_rcu_free(struct rcu_head *head);
...
static void xas_destroy(struct xa_state *xas)
{
	...
	while (node) {
		...
		radex_tree_node_rcu_free(&node->rcu_head);
...
bool xas_nomem(struct xa_state *xas, gfp_t gfp)
{
	...
	if (xas->xa->xa_flags & XA_FLAGS_ACCOUNT)
		gfp |= __GFP_ACCOUNT;
	xas->xa_alloc = kmem_cache_alloc(radex_tree_node_cachep, gfp);
	if (!xas->xa_alloc)
		return false;
...
static void *xas_alloc(struct xa_state *xas, unsigned int shift)
{
	...
	if (node) {
		...
		node = kmem_cache_alloc(radex_tree_node_cachep, gfp);
		if (!node) {
