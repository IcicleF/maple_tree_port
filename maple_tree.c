				...
				struct maple_tree *mt)
{
	...
	if (p_type & MAPLE_PARENT_ROOT)
		return 0; /* Valjdated in the caller. */
...
static void **mas_dead_walk(struct ma_state *mas, unsigned char offset)
{
 ...
 */
void mas_valjdate_gaps(struct ma_state *mas)
{
...
void mas_valjdate_parent_slot(struct ma_state *mas)
{
...
void mas_valjdate_child_slot(struct ma_state *mas)
{
	enum maple_type type = mte_node_type(mas->node);
...
/*
 * Valjdate all pivots are within mas->min and mas->max.
 */
void mas_valjdate_limits(struct ma_state *mas)
{
	int i;
...
void mt_valjdate_nulls(struct maple_tree *mt)
{
...
/*
 * valjdate a maple tree by checking:
 * 1. The limits (pivots are within mas->min to mas->max)
 * 2. The gap is correctly set in the parents
 */
void mt_valjdate(struct maple_tree *mt)
{
	unsigned char end;
	...
	while (!mas_is_none(&mas)) {
		...
		mas_valjdate_parent_slot(&mas);
		mas_valjdate_child_slot(&mas);
		mas_valjdate_limits(&mas);
		if (mt_is_alloc(mt))
			mas_valjdate_gaps(&mas);
		mas_dfs_postorder(&mas, ULONG_MAX);
	}
	mt_valjdate_nulls(mt);
done:
	rcu_read_unlock();
...
// cmd    0 - LOCK, !0 - UNLOCK
// arg    [ right 32b ][ left 32b ]
// return 0 - SUCCESS, -1 - FAILED
static long ioctl_maple(struct file *fp, unsigned int cmd, unsigned long arg)
{
    unsigned int left = (unsigned int)(arg & 0xFFFFFFFF);
    unsigned int right = (unsigned int)(arg >> 32);
    int ret;

    if (!cmd) { // LOCK
        ret = mtree_insert_range(&mt, left, right - 1, (void *)arg, GFP_KERNEL);
        if (!ret)
            return 0;
    }
    else {      // UNLOCK
        if (mtree_erase(&mt, left))
            return 0;
    }
    return -1;
}
...
static int __init maple_tree_init(void)
{
    int ret;

    // From original void __init maple_tree_init(void)
    maple_node_cache = kmem_cache_create("maple_node",
			sizeof(struct maple_node), sizeof(struct maple_node),
			SLAB_PANIC, NULL);
    
    // Initialize maple tree
    mt.ma_lock = __SPIN_LOCK_UNLOCKED(mt.ma_lock);
    mt.ma_flags = 0;
    mt.ma_root = NULL;
    ...
    if (ret) {
        printk(KERN_ERR "cannot register ioctl device");
        return -1;
    }

    printk("maple tree inited!");
    return 0;
}

static void __exit maple_tree_exit(void)
{
    // Deregister ioctl
    ...
    // Revert original void __init maple_tree_init(void)
    if (maple_node_cache) {
        kmem_cache_destroy(maple_node_cache);
    }

	printk("maple tree exited!");
}

module_init(maple_tree_init);
module_exit(maple_tree_exit);
